...
import { generateVehiclePartName } from '../naming';
...
export function VehicleScreen({data,setData}:{data:AppData;setData:(d:AppData)=>void}) {
  ...
  const clonePart=(part:VehiclePart)=>{const c=structuredClone(part);c.id=crypto.randomUUID();c.name=generateVehiclePartName(c.baseName,c);c.createdAt=c.updatedAt=new Date().toISOString();setPartMode('add');setPartEditor(c);};
  const savePart=(input:VehiclePart)=>{const part={...input,name:generateVehiclePartName(input.baseName,input)};const issue=validatePart(part);if(issue){alert(issue);return;}const now=new Date().toISOString();part.updatedAt=now;if(!part.createdAt)part.createdAt=now;setData({...data,vehicleParts:partMode==='add'?[...data.vehicleParts,part]:data.vehicleParts.map(p=>p.id===part.id?part:p)});setPartEditor(null);};
...
function VehiclePartEditor({item,mode,onClose,onSave}:{item:VehiclePart;mode:'add'|'edit';onClose:()=>void;onSave:(p:VehiclePart)=>void}) {
  const [v,setV]=useState({...item,name:generateVehiclePartName(item.baseName,item)});
  const patch=(next:Partial<VehiclePart>)=>setV(current=>{const updated={...current,...next};return {...updated,name:generateVehiclePartName(updated.baseName,updated)};});
  return <div className="modalBackdrop"><div className="modal sheet"><div className="modalHeader"><div><span className="eyebrow">{mode==='add'?'NEW VEHICLE PART':'EDIT VEHICLE PART'}</span><h2>{v.name||v.slot}</h2></div><button className="iconButton" onClick={onClose}>×</button></div><div className="formGrid"><label className="wide">Actual part name<input autoFocus value={v.baseName} onChange={e=>patch({baseName:e.target.value})}/></label><div className="wide autoNamePreview"><span>Automatic name</span><strong>{v.name||'Enter the part name'}</strong></div><label>Slot<select value={v.slot} onChange={e=>patch({slot:e.target.value as VehiclePartSlot})}><option>Spoiler</option><option>Exhaust</option><option>Wheel Stack</option></select></label><label className="check"><input type="checkbox" checked={v.favourite} onChange={e=>patch({favourite:e.target.checked})}/> Lock / favourite</label><NumberInput label="Vehicle Speed %" value={v.speedPct} onChange={n=>patch({speedPct:n})}/><NumberInput label="Vehicle Acceleration %" value={v.accelerationPct} onChange={n=>patch({accelerationPct:n})}/><NumberInput label="Weight Capacity kg" value={v.capacityKg} onChange={n=>patch({capacityKg:n})}/><NumberInput label="Handling % (only if shown)" value={v.handlingPct} onChange={n=>patch({handlingPct:n})}/><label>Mutation / prefix<input value={v.mutation??''} onChange={e=>patch({mutation:e.target.value})}/></label><label>Rarity<input value={v.rarity??''} onChange={e=>patch({rarity:e.target.value})}/></label></div><div className="notice">Type the actual part name once. The app automatically appends every non-zero performance stat, so duplicate rolls are easy to distinguish. Positive and negative decimal rolls are allowed.</div><div className="stickyActions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave({...v,name:generateVehiclePartName(v.baseName,v)})}>{mode==='add'?'Add part':'Save changes'}</button></div></div></div>;
}

function TrailerEditor({item,mode,onClose,onSave}:{item:Trailer;mode:'add'|'edit';onClose:()=>void;onSave:(t:Trailer)=>void}) {
  const [v,setV]=useState(item);
  return <div className="modalBackdrop"><div className="modal sheet"><div className="modalHeader"><div><span className="eyebrow">{mode==='add'?'NEW TRAILER':'EDIT TRAILER'}</span><h2>{v.name||'Trailer'}</h2></div><button className="iconButton" onClick={onClose}>×</button></div><div className="formGrid"><label className="wide">Name<input autoFocus value={v.name} onChange={e=>setV({...v,name:e.target.value})}/></label><NumberInput label="Trailer capacity kg" value={v.capacityKg} onChange={n=>setV({...v,capacityKg:n})}/><label className="check"><input type="checkbox" checked={v.favourite} onChange={e=>setV({...v,favourite:e.target.checked})}/> Lock / favourite</label></div><div className="notice">Trailer capacity is applied before the vehicle capacity multiplier, matching the in-game Long Trailer test.</div><div className="stickyActions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave(v)}>{mode==='add'?'Add trailer':'Save changes'}</button></div></div></div>;
}
...
function blankPart(slot:VehiclePartSlot):VehiclePart { const now=new Date().toISOString();return{id:crypto.randomUUID(),baseName:'',name:'',slot,speedPct:0,accelerationPct:0,handlingPct:0,capacityKg:0,favourite:false,createdAt:now,updatedAt:now}; }
...
function validatePart(p:VehiclePart) { if(!p.baseName.trim())return 'Part name is required.';if(!['Spoiler','Exhaust','Wheel Stack'].includes(p.slot))return 'Valid part slot is required.';for(const [label,value] of [['Speed',p.speedPct],['Acceleration',p.accelerationPct],['Handling',p.handlingPct],['Capacity',p.capacityKg]] as const)if(!Number.isFinite(value))return `${label} must be a finite number.`;return ''; }
